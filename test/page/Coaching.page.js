...
class selectorsRegister extends Page {
    ...
    async openApp(path) {
        await browser.url(`https://software-testers.gitlab.io/challenges/automation-challenges/${path}.html`)
    }

    async addSpecificLengthValue(data, dataLength) {
        let value = `${data}`.repeat(`${dataLength}`)
        console.log(value)
        await this.firstName.addValue(value)
    }
    async verifyErrorMessageExceptVerifyMe(messageText) {
        ...
        for (let i = 0; i < selectorName.length; i++) {
            ...
            await expect(this.confMessage).toHaveTextContaining(`${messageText}`)
        }
    }
}
